...
WordGame.prototype.addResponseTitles=function(a){var b=makeElement("span",{"class":"response",id:"rlrp",title:"Right letter right place. A letter could be in the first position in both words, for example."});b.innerHTML="rlrp";a.insertBefore(b,a.childNodes[0]);b=makeElement("span",{"class":"response",id:"rlwp",title:"Right letter wrong place. A letter could be in the first position in the word and the second position in the guess, for example."});b.innerHTML="rlwp";a.insertBefore(b,a.childNodes[1])};
...
1==game.numberOfGuesses()?(game.addResponseTitles(game.gameContainer),game.addInputs(game.gameContainer),game.animateFromFirstGuess()):($(a).addClass("hidden-game"),game.animateGuess(a)),a;else{var e;$("#popup").popup({content:function(){return buildPopup("Word is invalid!",c+"<br /><br />game closes automatically in 3 seconds","Continue","$('#popup').data('popup').close();")},beforeOpen:function(){$("#"+game.gameName+"-guess-input").blur();$(document).on("keydown",function(a){13==a.keyCode&&$("#popup").data("popup").close()})},
...
"eliminated";game.wordRows.push(b);game.gameContainer.insertBefore(b,game.gameContainer.childNodes[game.gameContainer.childNodes.length-1]);$(".word-col").draggable({helper:function(a){a=$(this).clone().appendTo(game.gameEl);$(this).toggleClass("ghost");return a},stop:function(a,b){$(this).toggleClass("ghost")},distance:20});game.updateColors();return b};WordGame.prototype.extraEnterGuess=function(){};
WordGame.prototype.animateGuess=function(a){$("#"+game.gameName+"-game-wrapper").animate({height:$("#"+game.gameName+"-game-wrapper").height()+$(".word-row").first().outerHeight()+("time"==game.gameName?parseInt($("#time-bottom-guess-inputs").css("margin-bottom")):0)+"px"},750);$("#"+game.gameName+"-bottom-guess-inputs").animate({bottom:"-"+$(".word-row").first().outerHeight()+"px"},750,function(){$("#"+game.gameName+"-game-wrapper").css("height","auto");$(a).fadeIn().removeClass("hidden-game");$("#"+
game.gameName+"-bottom-guess-inputs").css("bottom","0px")})};WordGame.prototype.animateToFirstguess=function(){$("#"+game.gameName+"-game").animate({height:$("#"+game.gameName+"-first-guess-wrapper").actualHeight()+"px","padding-top":"24px","padding-bottom":"20px"},function(){$("#"+game.gameName+"-first-guess-wrapper").fadeIn().removeClass("hidden-game");game.focusInput()})};
WordGame.prototype.animateFromFirstGuess=function(){$("#"+this.gameName+"-first-guess-wrapper").animate({opacity:"0"},function(){$("#"+game.gameName+"-first-guess-wrapper").remove();$("#"+game.gameName+"-game").animate({height:"0px","padding-top":"0px","padding-bottom":"0px"},function(){game.expandContainer()})})};
...
function clearSave(){if(0!=canLocalStorage()){var a=localStorage.getItem(waves("gameSettings"));localStorage.clear();localStorage.setItem(waves("gameSettings"),a)}}WordGame.prototype.continueGame=function(){var a=document.getElementById(game.gameName+"-game-wrapper");game.addResponseTitles(a);game.addInputs(a);for(a=0;a<game.numberOfGuesses();a++)game.guessFeedback(game.guesses[a]);for(a=0;a<$(".holder").length;a++)$(".holder")[a].innerHTML=game.specialLetters.top[a];game.expandContainer()};
function RegularGame(a){this.base=WordGame;this.base("regular")}RegularGame.prototype=new WordGame;function EasyGame(a){this.base=WordGame;this.base("easy");this.line='<span class = "easy-word-col"></span><span class = "easy-word-col"></span><span class = "easy-word-col"></span><span class = "easy-word-col"></span><span class = "easy-word-col"></span></span>'}EasyGame.prototype=new WordGame;EasyGame.prototype.addResponseTitles=function(a){};
...
game.gameContainer.insertBefore(b,game.gameContainer.childNodes[game.gameContainer.childNodes.length-1]);game.saveGame();return b};EasyGame.prototype.clickHolder=function(){};EasyGame.prototype.updateColors=function(){};function TimeGame(a){this.base=WordGame;this.base("time");this.maxGuesses=15;this.reloadMessage="Progress will be LOST if you leave or reload the game!"}TimeGame.prototype=new WordGame;
